...
const CheckoutPage = () => {
  ...
  const calculateDiscountAmount = () => {
    ...
    if (selectedDiscounts.SHIPPING) {
      ...
      // Giới hạn shippingFreeDiscount không vượt quá shippingFee
  ...
  useEffect(() => {
    const fetchAvailableDiscounts = async () => {
      try {
        ...
        console.log("Mã giảm giá nhận được:", response);
        setAvailableDiscounts(response || []);
        // Reset selectedDiscounts nếu không có mã giảm giá khả dụng
  ...
  const handleConfirmCheckout = async () => {
    ...
    try {
      ...
      // Đảm bảo giá trị giảm giá là 0 nếu không có mã giảm giá
      ...
      // Tạo mảng discountIds từ selectedDiscounts
      const discountIds = [];
      if (selectedDiscounts.TOTAL) {
      ...
      // Xử lý phản hồi dựa trên phương thức thanh toán
      if (paymentMethod === "VNPAY" && response.data.paymentUrl) {
        // Chuyển hướng đến paymentUrl cho VNPAY
        window.location.href = response.data.paymentUrl;
      } else {
        // Xử lý cho COD hoặc các trường hợp khác
        notification.success({
          message: "Thông báo",
