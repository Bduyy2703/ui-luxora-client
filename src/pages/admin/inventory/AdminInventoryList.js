...
const AdminInventoryList = () => {
  ...
  const checkLocationFormat = (location) => {
    if (!location) return false;

    const regex = /^Số\s+\d+,\s*[Đđ]ường\s+[^,]+,\s*[^,]+$/;

    if (!regex.test(location.trim())) return false;

    const parts = location.split(",").map((part) => part.trim());

    if (parts.length < 3) return false;

    if (!parts[0].match(/^Số\s+\d+$/)) return false;

    if (!parts[1].match(/^[Đđ]ường\s+/)) return false;
                ...
                rules={[
                  ...
                  },
                  {
                    validator: async (_, value) => {
                      ...
                      if (checkLocationFormat(value)) {
                        return Promise.resolve();
                      }
                      return Promise.reject(
                        new Error(
                          "Vui lòng nhập địa chỉ theo định dạng: Số nhà, tên đường, tên tỉnh/thành phố!",
                        ),
                      );
                    },
                  },
