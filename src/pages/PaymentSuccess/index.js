    ...
    const processPayment = async () => {
      try {
        ...
        if (!invoiceId) {
          console.log("Missing invoiceId");
          setResult("Thiếu ID hóa đơn.");
          setIsLoading(false); // Cập nhật isLoading
          return;
        }
        ...
        const response = await axios.get(
          `https://www.dclux.store/api/v1/invoices/${invoiceIdNum}`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
        ...
        if (response.data.status === "PAID") {
          console.log("Payment successful");
          setResult(
            `Thanh toán thành công! Hóa đơn #${invoiceId}, tổng: ${response.data.finalTotal} VNĐ`,
          );
        } else {
          console.log("Invoice status not PAID:", response.data.status);
          setResult(
            `Hóa đơn chưa được thanh toán. Trạng thái: ${response.data.status}`,
          );
          setTimeout(() => navigate("/payment-fail"), 3000);
        }
      } catch (error) {
        ...
        setIsLoading(false); // Đặt isLoading thành false sau khi API hoàn tất
      }
    };
      ...
      >
        {isLoading ? (
          <div style={{ marginBottom: "16px" }}>
            ...
            <p style={{ color: "#4b5563", marginTop: "8px" }}>Đang xử lý...</p>
          </div>
        ) : invoice && invoice.status === "PAID" ? (
          <>
            <div style={{ marginBottom: "16px" }}>
            ...
            <div style={{ textAlign: "left", marginBottom: "24px" }}>
              ...
              <p style={{ color: "#374151" }}>
                <span style={{ fontWeight: "600" }}>Trạng thái:</span> Đã thanh
                toán
              </p>
            </div>
          </>
          ...
          <>
            ...
            <p style={{ color: "#4b5563", marginBottom: "16px" }}>
              {result || "Vui lòng kiểm tra lại hóa đơn."}
            </p>
          </>
        )}
        {!isLoading && (
